...
│   ├── novel_compression_generator.py  # Invertible compression primitives, pipelines & incremental recipe evolution
...
│   │   ├── individual_agent.py     # EvolvingAgent wrapper with heritage lineage, agent types & novel recipe evolution
...
│   ├── benchmark_sets/
│   ├── github_cache/               # Cached GitHub files for phased training
│   ├── models/
│   ├── recipe_vault.json           # (v0.9.9) Top 5 proven recipes persisted across runs
│   └── method_registry.json        # (v0.9.10) All recipe families ever discovered (alive + graveyard)
├── logs/                           # Runtime logs
...
| -- | `set_overrides()` | Sets persistent difficulty floor (POST /api/training/set-overrides) — accepts `benchmark_size_kb` (int, 1-10240, null=clear) and `complexity_pct` (int, 0-100, null=clear). Works before or during training: stores on `AppState` and applies to optimizer if running; pending floors are applied when optimizer is created. Floors set minimums; AI can still advance beyond them via gold-standard gates. |
...
| -- | `start_queue()` | Starts a sequential multi-override training sweep (POST /api/training/start-queue) — accepts `steps[]` list, each with `generations`, `benchmark_size_kb` (null=auto), `complexity_pct` (null=auto), `label`. First step creates fresh optimizer (like /start), subsequent steps continue evolution (like /continue) with new overrides applied. Also accepts `population_size`, `batch_size`, `target_device`, `cpu_workers`. Queue progress tracked via `AppState.run_queue_*` fields and reported in `/api/status`. |
| -- | `queue_status()` | Returns current run queue progress (GET /api/training/queue-status) — `active`, `current_step`, `total_steps`, `remaining[]` |
| ~355 | `evolution_deep_dive()` | Returns enriched generation data for deep-dive tab (GET /api/evolution/deep-dive) — gene pool clustering, enriched breeding pairs (with parent/child fitness, pool indices, cross-pool flag), pool-to-pool breeding matrix, top breeders (most selected parents), method direction trends (avg RLE min run, novel method count, cross-pool stats per gen), mutation/crossover counts, top agents, per-agent lineage. **Dual-scored pools** (v0.9.9): each pool summary includes `avg_compression`, `avg_robustness`, `best_compression`, `best_robustness`, `compression_agents`, `robustness_agents`, `specialization` ('compression'/'robustness'/'hybrid'). Per-agent enrichment includes `agent_type`, `compression_fitness`, `robustness_fitness` from `agent_detail_map`. **v0.9.10 recipe visibility**: per-agent enrichment adds `recipe_strength`, `recipe_family`, `recipe_is_alive`, `recipe_times_rediscovered`. Per-pool summary adds `recipe_families` (family→count dict), `dominant_family`, `avg_recipe_strength`, `mature_recipe_count`, `alive_recipe_count`, `resurrected_agent_count`. `method_direction` adds per-gen `avg_recipe_strength`, `mature_recipe_count`, `unique_families`, `resurrected_count`, `dominant_family`. Top-level response includes `method_registry` summary (total/alive/dead families, total rediscoveries, top families list) |
...
| `webui_static/js/app.js` | `PuffinZipAIApp` class — 3-dataset chart (compression score + benchmark + complexity), complexity polling, log filtering, export CSV/TXT, population history viewer, top-level tab switching. Compression score displayed as compression factor `orig/comp*100` (>100% = actual compression). **Continue training**: `continueTraining()` sends POST to `/api/training/continue`, `canContinue` flag tracked from `/api/status`, `updateUIState()` enables Continue button when a run completes naturally. **Difficulty floor**: `applyOverrides()` sends POST to `/api/training/set-overrides` with `benchmark_size_kb` and `complexity_pct` as persistent minimums; `clearOverrides()` resets the UI inputs. Works before or during training. **Run queue**: `rqAddStep()`, `rqRemoveStep()`, `rqClear()`, `rqStart()` manage a list of sequential training steps with different overrides; `_rqRender()` shows the step list, `_rqUpdateProgress()` updates a progress indicator from `/api/status` polling. Chart is archived via `archiveAndResetChart()` when Start or Queue is pressed after a previous run. |
| `webui_static/js/deep_dive.js` | `DeepDiveManager` class — Polls `/api/evolution/deep-dive`, renders: interactive gene pool grid (click agent for **cross-generation lineage highlighting** of parents/children/siblings), lineage detail panel (multi-generational ancestry: parents, grandparents, great-grandparents, children, siblings — with clickable navigation to their generation), fitness/mutation chart, method direction chart (RLE trend + cross-pool breeds + novel methods + **v0.9.10 avg recipe strength dashed line** + **unique families bar chart**), gene pool legend with **specialization icons** (📦 compression / 🛡️ robustness / 🔀 hybrid) and **dual fitness scores** (C: avg_compression, R: avg_robustness) per pool + **v0.9.10 recipe badges** (💪 avg strength, 🧬 mature count, 🧟 resurrected count, top-3 family mini-bars), top agents leaderboard, breeding flow SVG (parent→child connection curves, cross-pool dashed lines), pool×pool breeding matrix (heatmap), top breeders list (frequency bars), **v0.9.10 method direction summary cards (6 cards)**: Avg Recipe Strength (progress bar), Novel Methods (current + total), Method Families (active + dominant), RLE Trend, Cross-Pool Breeds, Method Registry (total families, 🟢alive, 💀graveyard, 🧟resurrections). Agent tooltip enhanced with **method section**: family key, strength progress bar (color-coded: green≥70%, amber≥30%, red<30%), ✨NOVEL badge, 🧟resurrected badge. Per-generation expandable detail cards with enriched breeding pairs (fitness comparison, outcome indicators) and mini fitness bar charts. |
...
**Lines**: ~1660  
**Purpose**: Generates invertible compression methods from composable primitives + random discovery transforms. **v0.9.9**: Adds incremental recipe-based novel method evolution — novel methods are built slowly across generations through partial mutations, not randomly assigned per-agent. **v0.9.10**: Adds LoRA-like recipe strength, method registry/graveyard, breeding-out mechanics, and dormant re-emergence.
...
| ~520 | `RecipeStep` | Dataclass — single transform step with `step_type`, `params`, `contribution_score`, `generation_added` |
| ~530 | `NovelMethodRecipe` | Dataclass — mutable, evolvable compression recipe. Steps list, improvement log, maturity flag (`is_mature`=True after ≥2 proven improvements). **v0.9.10 fields**: `strength` (0.0–1.0, LoRA-like influence weight, default 0.5), `is_alive` (bool, `False` when bred out), `death_generation` (int or None), `times_rediscovered` (count of graveyard resurrections), `total_generations_active` (lifetime counter). **v0.9.10 properties**: `is_weak` (strength < 0.1), `family_key` (step-type signature string for deduplication), `aggregate_contribution` (sum of step contribution scores). Serializable via `to_dict()`/`from_dict()`. `to_legacy_metadata()` for backward-compat with old `CompressionMethod.metadata` format |
| ~700 | `RecipeEvolver` | Static methods for incremental recipe evolution: `create_base_recipe()` (simple RLE-only starting point), `mutate_recipe()` (one small random change — add/remove/swap/modify/reorder step), `record_improvement()` (log successful mutation + update contribution scores + **v0.9.10 strength boost**: `min(delta*0.25, 0.3)` gain capped at 1.0), `should_combine()` (detect cross-family breeding), `combine_recipes()` (blend steps from two families at random weights; **v0.9.10**: child inherits averaged parent strength), `build_from_recipe()` (convert recipe → compress/decompress closures). **v0.9.10 methods**: `decay_strength(recipe, gen)` (base 0.03/gen decay, +0.02*(gens_stagnant-5) penalty after 5 stagnant gens, 2× faster for immature recipes; returns True if strength hit 0 = dead), `resurrect_recipe(dead_recipe, generation)` (deep-copies dead recipe, new ID, strength=0.4, increments `times_rediscovered`, logs resurrection in improvement_log) |
| ~1130 | `MethodRegistry` | **(v0.9.10)** Persistent JSON catalogue of all recipe families ever discovered. Stored at `data/method_registry.json`. Tracks per-family: `family_key`, `first_seen_gen`, `last_seen_gen`, `peak_strength`, `alive`/`dead` status, `agent_count`, `total_improvements`, `times_rediscovered`, `death_generations[]`. Methods: `register(recipe)`, `register_death(recipe, gen)`, `get_dead_recipes()` (returns `NovelMethodRecipe` objects from graveyard), `pick_graveyard_recipe()` (fitness-weighted random selection from dead families), `get_summary()` (total/alive/dead families + top families for API), `save()`. Auto-loads on init, auto-creates `data/` dir if missing |
| ~960 | `NovelCompressionGenerator` | Pipeline builder — `generate_novelty_method()`, `generate_random_discovery()`, `evolve_methods()`, `_build_pipeline()`, `_build_pipeline_from_steps()`, `build_method_from_recipe()`, `_verify_invertibility()` |

**Recipe Evolution Flow** (v0.9.9 + v0.9.10):
1. All agents start with same base recipe: `["rle"]` with `min_run=3`
2. During breeding, child inherits dominant parent's recipe + **one small mutation**
3. If parents have structurally different recipes → **combine** steps from both families
4. After evaluation, if fitness improved → mutation recorded as proven improvement + **strength boosted**
5. Steps accumulate contribution scores based on successful improvements
6. Recipe is genuinely "novel" only after ≥2 proven improvements (`is_mature`)
7. Heritage carries recipe data so descendants inherit proven improvements
8. **(v0.9.10) Strength decay**: every gen, all recipes lose strength (base 0.03/gen + stagnation penalty). Recipes reaching strength 0 are **bred out** — replaced with fresh base recipe
9. **(v0.9.10) Registry archival**: dead recipes are archived to `MethodRegistry` graveyard with full stats preserved
10. **(v0.9.10) Dormant re-emergence**: 5% chance per child during breeding to resurrect a graveyard recipe (fitness-weighted selection) — like a distant gene resurfacing
...
| ~360 | `_snapshot_generation()` | Captures lightweight per-generation population snapshot (agent summaries, batch grouping) for GDV history; sends `GEN_SNAPSHOT:<gen>` via queue. **v0.9.9**: each agent summary now includes `agent_type`, `compression_fitness`, `robustness_fitness` for dual-axis pool scoring. **v0.9.10**: adds `recipe_strength`, `recipe_family`, `recipe_is_alive`, `recipe_times_rediscovered` per agent for method lifecycle tracking |
...
| 303 | `_create_initial_population()` | Parallel agent creation with deferred GPU init + bulk GPU finalization + **50/50 agent type split** (first half "compression", second half "anti_corruption"). Types survive checkpoint/restart via EvolvingAgent `__setstate__` migration. **(v0.9.9) Recipe vault seeding**: loads `_load_recipe_vault()` and assigns proven recipes to the first N agents (distributed across both types via round-robin) instead of the default base recipe; remaining agents get `RecipeEvolver.create_base_recipe()` |
...
| -- | `set_manual_overrides(benchmark_size_kb, complexity_pct)` | Sets persistent difficulty floor; delegates to `BenchmarkItemEvaluator.set_manual_overrides()`; floors remain active until explicitly cleared (pass None) |
...
| Line | Name | Description |
|------|------|-------------|
| 51 | `EvolvingAgent` | Wraps a `PuffinZipAI` instance with evolution metadata, heritage lineage, agent type, dual fitness, and novel recipe |

**EvolvingAgent Attributes**: `puffin_ai`, `agent_id` (uuid), `fitness`, `generation_born`, `parent_ids`, `evaluation_stats`, `heritage` (list of ancestor trick dicts), `agent_type` ("compression" or "anti_corruption"), `compression_fitness` (float), `robustness_fitness` (float), `novel_recipe` (NovelMethodRecipe | None), `_pending_recipe_change` (dict | None — transient mutation tracking)

**EvolvingAgent Methods**:
| Method | Description |
|--------|-------------|
| `clone()` | Deep-clones agent including heritage, agent_type, and novel_recipe |
...
| `record_trick(...)` | Records a proven pipeline into heritage (bounded by MAX_HERITAGE_ENTRIES). v0.9.9: also stores recipe dict if available |
...
| `check_recipe_improvement(generation)` | **(v0.9.9)** Post-evaluation: if pending mutation improved fitness over recipe's best, record it as a proven improvement via `RecipeEvolver.record_improvement()` |
| `has_mature_novel_method` | **(v0.9.9 property)** True only when the agent's recipe has ≥2 proven improvements |
| `__getstate__()` | Pickle — serializes recipe as dict via `_novel_recipe_dict`, strips transient `_pending_recipe_change` |
| `__setstate__(state)` | Pickle — restores recipe from dict, provides defaults for pre-v0.9.7, pre-v0.9.9, and pre-v0.9.10 fields |

**Pickle Behavior**: `__getstate__` serializes novel_recipe as `_novel_recipe_dict` (plain dict), strips `_pending_recipe_change` (transient). `__setstate__` restores recipe via `NovelMethodRecipe.from_dict()` and migrates pre-v0.9.7/v0.9.9 checkpoints with sensible defaults. **v0.9.10**: after recipe deserialization, force-fixes missing `strength` (0.5), `is_alive` (True), `death_generation` (None), `times_rediscovered` (0), `total_generations_active` (0) on deepcopied recipe objects from older format.

**Heritage Entry Dict Schema**: `{"trick": str, "pipeline": str, "discovery_seed": int, "rle_min_run": int, "fitness_when_learned": float, "ancestor_id": str, "generation": int, "recipe": dict | None (v0.9.9)}`

**Key Dependencies**: `..ai_core` (PuffinZipAI), `..novel_compression_generator` (RecipeEvolver, NovelMethodRecipe), `uuid`, `copy`, `traceback`
...
## Gene Pool Architecture (v0.9.9)

This section documents how gene pools are structured, scored, and protected in the evolutionary system.

### Q1: How are we currently structuring/storing the mutation gene pool?

Gene pools are **implicit lineage clusters** — there is no explicit `GenePool` class or data structure. Pool membership is computed on-the-fly by tracing each agent's `parent_ids` chain back to the **root ancestor** (the first agent in the lineage with `parent_ids == []`). All agents sharing the same root ancestor belong to the same gene pool.

**Per-agent mutation state** is stored across three layers:

| Layer | Location | What it stores |
|-------|----------|----------------|
| **Q-table / NN weights** | `EvolvingAgent.puffin_ai` | Learned compression strategies (the "brain") |
| **Novel recipe** | `EvolvingAgent.novel_recipe` (`NovelMethodRecipe`) | Incremental compression method pipeline — list of `RecipeStep` objects with an `improvement_log` tracking which mutations were proven beneficial |
| **Heritage** | `EvolvingAgent.heritage` (list of dicts) | "Grandpapi" lineage memory — up to 32 proven tricks from ancestors, merged during breeding |

**Pool identity inheritance**: When two agents breed, each child inherits `parent_ids = [parent1.agent_id, parent2.agent_id]`. The child's root ancestor (and thus pool membership) is determined by the first parent's root. Founder agents (injected to restore diversity) have `parent_ids = []`, making them new root ancestors and thus new gene pools.

**WebUI computation**: `webui_server.py` computes "living gene pools" — only pools that have at least one member in the current generation's population are counted and displayed. Dead lineages (all descendants eliminated by selection) are pruned from the visualization.

### Q2: How are we scoring "superior" genes for the pool?

Scoring operates at **two levels**: individual agent fitness and pool-level aggregation.

**Individual agent scoring** (dual-axis):

| Agent Type | Primary Score | How Computed |
|------------|--------------|--------------|
| `"compression"` | `compression_fitness` | Evaluated on clean benchmark data — measures compression ratio quality |
| `"anti_corruption"` | `robustness_fitness` | Evaluated on corrupted/noisy benchmark data — measures data integrity under noise |

Both types also receive a combined `fitness` score (always equals clean compression fitness) used for overall population ranking.

**Pool-level scoring** (computed in `webui_server.py` deep-dive API):

| Metric | Meaning |
|--------|---------|
| `avg_compression` | Mean `compression_fitness` across all living pool members |
| `avg_robustness` | Mean `robustness_fitness` across all living pool members |
| `best_compression` | Highest `compression_fitness` in the pool |
| `best_robustness` | Highest `robustness_fitness` in the pool |
| `specialization` | `'compression'` / `'robustness'` / `'hybrid'` based on member type majority |

**Selection pressure** (how "superior" genes win):

1. **Per-type elitism**: Top-1 compression agent and top-1 anti-corruption agent are always preserved (never eliminated).
2. **Tournament selection**: Parents chosen by fitness within type-specific sub-populations.
3. **Heritage recording**: Tricks are recorded only above a fitness threshold of 2.0. Compression agents record based on clean fitness; anti-corruption agents record only tricks that survive corruption.
4. **Recipe improvement tracking**: `check_recipe_improvement()` logs a mutation as "proven" only when post-evaluation fitness exceeds the recipe's historical best.

**Founder injection** protects both scoring dimensions independently:

- Compression and anti-corruption pool diversity are checked separately (`comp_pool_counts`, `anti_pool_counts`).
- If either dimension drops below 3 distinct pools, or one pool holds ≥70% of that type's population, new founder agents are injected for that specific type.
- Founders are seeded from same-type agents when possible (compression founders from compression agents, robustness founders from anti-corruption agents) so the Q-table initialization matches the scoring dimension.

### Q3: Should we tie the existing heritage system into EvolvingAgent, or keep it separate?

Heritage **is already a first-class attribute** of `EvolvingAgent` and should remain there. Here's why:

1. **Heritage is per-agent state**: Each agent's heritage is unique — it reflects that specific lineage's accumulated discoveries. Moving it to a global pool structure would lose the per-lineage specificity.

2. **Heritage flows through breeding**: During crossover, `EvolvingAgent.merge_heritage(parent1.heritage, parent2.heritage)` deduplicates and merges both parents' tricks. Because parents' heritage already contains their ancestors' entries, the child transitively inherits the full family tree ("grandpapi" pattern) without storing the entire agent graph.

3. **Heritage is serialized with the agent**: `__getstate__` / `__setstate__` serialize heritage alongside all other agent state, ensuring checkpoints capture the complete lineage memory.

4. **Heritage is bounded**: `MAX_HERITAGE_ENTRIES = 32` prevents unbounded memory growth. When the limit is exceeded, the weakest entries (lowest `fitness_when_learned`) are dropped.

5. **Heritage carries recipe data (v0.9.9)**: Each heritage entry optionally includes a `"recipe"` dict from the discovering agent's `novel_recipe`, enabling cross-family breeding to reconstruct ancestor recipes.

**Summary**: Heritage belongs on `EvolvingAgent` because it is per-agent lineage memory, not a global resource. The gene pool is an emergent property of many agents sharing a common root ancestor — it doesn't need its own heritage store.
...
| `set_manual_overrides(benchmark_size_kb, complexity_pct)` | Sets persistent difficulty floor — per-item size (KB) and/or complexity (0-100%) minimums; AI-driven values are raised to floor if below, never capped; pass None to clear |
...
|--------|----------|---------|
| `start.sh` (repo root) | Linux/macOS | **Universal launcher** — auto-detects hardware (CPU, RAM, GPU type & VRAM), clones repo if missing, creates venv, installs deps, generates credentials, reads `public_access` from credentials to determine bind address. Automatically loads `.env` (git-ignored) for local config overrides. Auto-detects RunPod (proxy URL) and Vast.ai; falls back to public IP detection for generic cloud/bare metal. **Auto-installs and starts Cloudflare Tunnel** on Linux — supports token-based auth (`CLOUDFLARE_TUNNEL_TOKEN`), base64 credentials (`CLOUDFLARE_TUNNEL_CREDS`), or local `~/.cloudflared/` credentials. Generates dynamic `config.yml` with the actual server port. Exports hardware profile env vars, starts WebUI. Run presets (Test / Medium / Max) are available in the WebUI. Env vars: `PUFFIN_GPUS`, `PUFFIN_WORKERS`, `PUFFIN_CACHE_MAX_MB`, `PUFFIN_CACHE_MAX_FILES`, `PUFFIN_PORT`, `PUFFIN_HOST`, `PUFFIN_REPO_URL`, `PUFFIN_REPO_BRANCH`, `GITHUB_TOKEN`, `PUFFIN_ADMIN_USERNAME`, `PUFFIN_ADMIN_PASSWORD`, `PUFFIN_CUSTOM_URL`, `CLOUDFLARE_TUNNEL_TOKEN`, `CLOUDFLARE_TUNNEL_CREDS`, `CLOUDFLARE_TUNNEL_NAME` |
| `start.bat` (repo root) | Windows | **Universal launcher** — same as `start.sh` for Windows. Auto-detects hardware, creates venv, installs deps, generates credentials, detects RunPod/cloud platform, resolves connect URL, auto-starts Cloudflare Tunnel (supports `CLOUDFLARE_TUNNEL_TOKEN` and local credentials), starts WebUI |
