...
| `webui_static/js/app.js` | `PuffinZipAIApp` class — 3-dataset chart (compression score + benchmark + complexity), complexity polling, log filtering, export CSV/TXT, population history viewer, top-level tab switching. Compression score displayed as compression factor `orig/comp*100` (>100% = actual compression). |
...
| `webui_static/js/charts.js` | Global helpers `addMetricToChart()` (3 datasets) + `resetChart()` |
...
| 524 | `start_evolution()` | Main evolution loop — evaluate → snapshot → breed → pre-generate next benchmark set → repeat; uses background threading to overlap benchmark data generation with evaluation/breeding |
...
| `determine_target_complexity(fitness, ratio, gold_standard_win_rate)` | **Progression-locked with dwell time + gold standard gating** — advances complexity at most **one tier at a time** AND requires **2 refreshes** at the current tier AND ratio gate AND gold standard win rate gate (`COMPLEXITY_GOLD_STANDARD_GATES`: SIMPLE ≥10%, MODERATE ≥30%, COMPLEX ≥50%, VERY_COMPLEX ≥70%; all tiers gated). When `gold_standard_win_rate < 0` (no data) and `gs_gate > 0`, advancement is **blocked** (not bypassed). Can **drop back multiple tiers in one refresh** when ratio falls below 75 % of each tier's gate. Tracks `_refreshes_at_current_tier` (reset on advance or drop). Only called from `generate_and_set_dynamic_benchmark_items()`. |
...
| `COMPLEXITY_RATIO_GATES` | Dict: SIMPLE=25%, MODERATE=45%, COMPLEX=60%, VERY_COMPLEX=70% — ratio gate for complexity tier advancement; 75% of gate value is the retention/drop threshold |
| `COMPLEXITY_GOLD_STANDARD_GATES` | Dict: SIMPLE=10%, MODERATE=30%, COMPLEX=50%, VERY_COMPLEX=70% — minimum fraction of items where AI must beat ALL baselines before complexity can advance; all tiers now gated (SIMPLE was 0%, now 10%); `gold_standard_win_rate < 0` (no data) blocks advancement when gate > 0 |
