#!/usr/bin/env bash
# ============================================================================
#  PuffinZipAI — A100 PCIe Pod Launcher
#  One-command setup + start. Run this every time.
#  Console stays open for live logs & debugging.
#
#  A100 PCIe — 40 GB VRAM, 312 TFLOPS TF32, 624 TFLOPS FP16 (w/ sparsity)
...
cat << 'EOF'
  ...
  ║       A100 PCIe · GPU+CPU Pipeline · DQN Neural Agents  ║
  ╠══════════════════════════════════════════════════════════╣
...
if command -v nvidia-smi &>/dev/null; then
    ...
    if [[ "$GPU_NAME" != *"A100"* ]]; then
        warn "Expected A100 but found '$GPU_NAME' — script will still work"
    fi
else
...
echo -e "  ${CYAN}GPU:${NC}     ${BOLD}A100 PCIe${NC}"
