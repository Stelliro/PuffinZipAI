#!/usr/bin/env bash
# ============================================================================
#  PuffinZipAI — A40 Pod Launcher (Multi-GPU + Multi-CPU)
#
...
cat << 'EOF'
  ...
  ╔══════════════════════════════════════════════════════════╗
  ║             🐧  PuffinZipAI  v0.9.7                     ║
  ║       A40 · Multi-GPU · Multi-CPU · DQN Neural Agents   ║
  ╠══════════════════════════════════════════════════════════╣
...
if [[ "$GPU_COUNT" -gt 0 ]]; then
echo -e "  ${CYAN}GPUs:${NC}     ${BOLD}$GPU_COUNT × A40 (CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES)${NC}"
else
